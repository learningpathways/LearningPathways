...
(function (global, factory) {
    ...
    var Gauge = (function () {
        ...
        function getValueInPercentage(value, min, max) {
            var newMax = max - min, newVal = value - min; /* declaration changed to var to support ie10 */
...
jQuery(document).ready(function ($) {
    ...
    $('body.recording-page').css({ 'width': windowWidth, 'overflow-x': 'hidden' });
    ...
    // sidebar open on viewports larger than 480px
    $(window).on('load', function () {
        if (viewportWidth > MqS) {
    ...
    // open/close sidebar
    var localSidebarTrigger = $('.local-sidebar-trigger'),
           recordingSection = $('section.recording');
    localSidebarTrigger.on('click', function (event) {
        event.preventDefault();
        recordingSection.toggleClass('sidebar-open');
    });
    // close sidebar when click on in-page links
    $('.l-toc a').on('click', function () {
        if (viewportWidth < MqS) {
            recordingSection.toggleClass('sidebar-open');
        }
    });
    ...
    $('.has-dropdown a').on('click', function (event) {
        ...
        if (!selected.next('ul').hasClass('is-visible')) {
            ...
            selected.parent('.has-dropdown').siblings('.has-dropdown').children('ul').removeClass('is-visible').end().children('a').removeClass('selected');
        } else {
            selected.removeClass('selected').next('ul').removeClass('is-visible');
        }
    });
    //close dropdown if click outsite of nav
    $('html').on('click', function () {
        $('.has-dropdown a').removeClass('selected').next('ul').removeClass('is-visible');
    });
    ...
    // smooth scrool on in-page links
    ...
    //search expand when text is present
    $('#search').on('keyup', function () {
        $('.g-search--typeahead').addClass('is-visible');
        if ($('#search').val().length === 0) {
            $('.g-search--typeahead').removeClass('is-visible');
        }
    });
    ...
    //Sitemap trigger
    var sitemapTrigger = $('.sitemap-trigger');
    sitemapTrigger.on('click', function (event) {
        event.preventDefault();
        $('body').toggleClass('sitemap-open');

        if ($(window).width() < 480) {//viewportWidth < MqS) {
    ...
    //open video-modal
    //$('.video-modal-trigger').on('click', function (event) {
    //    event.preventDefault();
    ...
    $('.video-modal').on('click', function (event) {
        if ($(event.target).is('.video-modal-close') || $(event.target).is('.video-modal')) {
            event.preventDefault();
            try {
                var iframe = document.getElementById("content").getElementsByTagName("iframe");
    ...
    //close video-modal when clicking the 'esc' keyboard button
    ...
    $('.global-breadcrumbs nav ol').scrollLeft(9999);

    //$('#toast-dismiss').click(function (e) {
